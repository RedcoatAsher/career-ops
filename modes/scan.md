...
For companies on Greenhouse, the JSON API (`boards-api.greenhouse.io/v1/boards/{slug}/jobs`) returns clean structured data. Use as a quick supplement to Level 1 — faster than Playwright but only works with Greenhouse.
...
- Add companies to `tracked_companies` when they warrant close tracking
