...
7.5. **Liveness verification for WebSearch results (Level 3)** — BEFORE adding to pipeline:
   ...
   c. Classify:
      - **Active**: job title visible + role description + Apply/Submit button visible in main content. Do not count generic header/navbar/footer text.
...
Every company in `tracked_companies` must have `careers_url` — the direct URL to their job listings page. This avoids looking it up every time.
