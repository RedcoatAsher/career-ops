...
| 5 | location | string | `London` | City or region |
| 6 | remote | string | `remote` | `remote` or `on-site` |
| 7 | status | canonical | `Evaluated` | MUST be canonical (see states.yml) |
| 8 | score | X.XX/5 | `4.55/5` | Or `N/A` if not evaluable |
| 9 | pdf | emoji | `✅` or `❌` | Whether PDF was generated |
| 10 | report | md link | `[647](reports/647-...)` | Link to report |
| 11 | notes | string | `APPLY HIGH...` | One-sentence summary |
...
7. When writing English (PDF summaries, bullets, STAR stories), use native tech English: short sentences, action verbs, no unnecessary passive voice, no "in order to" or "utilized"
